...
function getComputedTranslateXY(obj) {
    ...
    let a = mat ? transArr.push(parseFloat(mat[1].split(', ')[4])) : 0;
    let b = mat ? transArr.push(parseFloat(mat[1].split(', ')[5])) : 0;
    return transArr;
}
...
function createBrife(pin){
    const imageURL_0 = pin.images[0].uri;
    // const imageURL_1 = pin.images[1].uri;
    // const imageURL_2 = pin.images[2].uri;
    const icon = pin.icon;
    ...
    var user = {};
    return db.collection("users").doc(pin.owner.toString()).get().then(doc =>{
        if (!doc.exists){
            console.log("No user");
        } else {
            const user= doc.data();
...
function renderMarker(marker) {
    ...
    let currentIcon = '';
    createBrife(marker).then(function(res){
        marker.des = res;
    });
